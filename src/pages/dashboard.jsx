...
export default function Dashboard() {
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white">
      {/* Sidebar */}
      <div className="fixed left-0 top-0 h-full w-64 bg-black/40 backdrop-blur-xl border-r border-white/10 p-6">
...
{showCreateModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
    <div className="relative w-full max-w-lg">
      ...
      <div className="relative bg-[#0a0a0a] border border-[#8A00C4]/40 rounded-lg shadow-2xl">
        ...
        <div className="px-5 py-3 border-b border-[#8A00C4]/30 flex justify-between items-center bg-gradient-to-r from-[#0a0a0a] via-[#111111] to-[#0a0a0a]">
          <input
            type="text"
            placeholder="Note title..."
            value={newNote.title}
            onChange={(e) => setNewNote({...newNote, title: e.target.value})}
            className="flex-1 bg-transparent text-white placeholder-gray-500 focus:outline-none font-medium focus:text-[#8A00C4] transition-colors"
          />
          <button 
            onClick={() => setShowCreateModal(false)} 
            className="p-1.5 rounded-full hover:bg-[#8A00C4]/10 text-gray-400 hover:text-white transition"
          >
            <FaTimes />
          </button>
        </div>
  ...
  <button
    ...
    className={`px-3 py-1.5 rounded text-sm hover:bg-white/10 ${editor?.isActive('bold') ? 'bg-purple-700' : 'text-gray-400'}`}
    title="Bold"
  >
    B
  </button>
  <button
    ...
    className={`px-3 py-1.5 rounded text-sm hover:bg-white/10 ${editor?.isActive('italic') ? 'bg-purple-700' : 'text-gray-400'}`}
    title="Italic"
  >
    I
  </button>
  ...
  <EditorContent
  editor={editor}
  className="min-h-[150px] bg-[#111111]/50 border border-[#8A00C4]/30 rounded px-3 py-2 text-white placeholder-gray-600 focus:outline-none focus:border-[#8A00C4] focus:ring-1 focus:ring-[#8A00C4]/30 transition text-sm"
/>
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl border border-white/10 shadow-2xl w-full max-w-md p-8">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 py-3 rounded-full border border-white/20 hover:bg-white/10 transition"
              >
                Cancel
              </button>
              <button
                onClick={() => {localStorage.removeItem("token");setUser(null);navigate('/')}}
                className="flex-1 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 py-3 rounded-full font-semibold transition-all"
              >
                Logout
              </button>
            </div>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
    <div className="relative w-full max-w-lg">
      ...
      <div className="relative bg-[#0a0a0a] border border-blue-500/40 rounded-lg shadow-2xl">
        ...
        {/* Compact Header with subtle glow */}
        <div className="px-5 py-3 border-b border-blue-500/30 flex justify-between items-center bg-gradient-to-r from-[#0a0a0a] via-[#111111] to-[#0a0a0a]">
          <input
            type="text"
            placeholder="Note title..."
            value={newNote.title}
            onChange={(e) => setNewNote({...newNote, title: e.target.value})}
            className="flex-1 bg-transparent text-white placeholder-gray-500 focus:outline-none font-medium focus:text-blue-400 transition-colors"
          />
          <button 
            onClick={() => {
              setShowEditModal(false);
            ...
            className="p-1.5 rounded-full hover:bg-blue-500/10 text-gray-400 hover:text-white transition"
          >
            <FaTimes />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <div className="flex gap-2 mb-2">
              <button
                onClick={() => editor?.chain().focus().toggleBold().run()}
                className={editor?.isActive('bold') ? 'bg-blue-700 px-2 rounded' : 'px-2 hover:bg-white/10 rounded'}
              >
                <FaBold />
              </button>
              <button
                onClick={() => editor?.chain().focus().toggleItalic().run()}
                className={editor?.isActive('italic') ? 'bg-blue-700 px-2 rounded' : 'px-2 hover:bg-white/10 rounded'}
              >
                <FaItalic />
              </button>
            </div>
            ...
            <EditorContent
              editor={editor}
              className="min-h-[150px] bg-[#111111]/50 border border-blue-500/30 rounded px-3 py-2 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition text-sm"
            />
          </div>
    ...
    <>
      <div className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition group relative">
        <div className="flex justify-between items-start mb-3">
          <h3 className="font-semibold text-lg truncate" style={{cursor:"pointer"}} onClick={()=>handleEdit(note)}>{note.title}</h3>
          ...
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 top-full mt-1 w-48 bg-[#1a1a1a] border border-white/10 rounded-lg shadow-xl z-10">
                <div className="p-1">
                  {/* Pin/Unpin Option */}
                  <button
                    onClick={handlePinToggle}
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm rounded hover:bg-white/5 text-white"
                  >
                    {note.pinned ? (
                      <>
                        <MdPushPin className="text-yellow-400" />
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm rounded hover:bg-white/5 text-red-400"
                  >
                    <FaTrash />
                    Delete Note
                  </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl border border-white/10 shadow-2xl w-full max-w-md p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400">Are you sure you want to delete "{note.title}"? This action cannot be undone.</p>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={handleDelete} // You'll need to implement this function
                className="flex-1 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 py-3 rounded-full font-semibold transition-all"
              >
                Delete
              </button>
